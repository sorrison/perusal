...
for dirpath, dirnames, filenames in os.walk('etc'):
    for i, dirname in enumerate(dirnames):
        if dirname.startswith('.'): del dirnames[i]
    if filenames:
        data_files.append([dirpath.replace('etc', '/etc/perusal'), [os.path.join(dirpath, f) for f in filenames]])
...
setup(
    name = "perusal",
    version = version,
    url = 'http://code.vpac.org/trac/perusal/',
    author = 'Sam Morrison',
    author_email = 'sam@vpac.org',
    description = 'Django survey web app',
    packages = packages,
    cmdclass = cmdclasses,
